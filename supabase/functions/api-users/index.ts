...
serve(async (req) => {
  ...
  try {
    ...
    if (req.method === 'GET' && pathParts.includes('me')) {
      ...
      if (error) {
        await logUsage(supabase, keyId, '/me', 'GET', 404, Date.now() - startTime, ipAddress);
        return errorResponse('NOT_FOUND', 'Profile not found', 404);
      }

      await logUsage(supabase, keyId, '/me', 'GET', 200, Date.now() - startTime, ipAddress);
    ...
    if (req.method === 'PUT' && pathParts.includes('me')) {
      ...
      if (error) {
        await logUsage(supabase, keyId, '/me', 'PUT', 500, Date.now() - startTime, ipAddress);
        return errorResponse('DATABASE_ERROR', error.message, 500);
      }

      await logUsage(supabase, keyId, '/me', 'PUT', 200, Date.now() - startTime, ipAddress);
      return successResponse(data);
    }
    ...
    if (req.method === 'GET' && pathParts.includes('search')) {
      ...
      if (!query || query.length < 2) {
        await logUsage(supabase, keyId, '/search', 'GET', 400, Date.now() - startTime, ipAddress);
      ...
      await logUsage(supabase, keyId, '/search', 'GET', 200, Date.now() - startTime, ipAddress);
    ...
    if (req.method === 'GET') {
      ...
      if (targetUserId && targetUserId !== 'api-users') {
        ...
        if (error) {
          await logUsage(supabase, keyId, `/${targetUserId}`, 'GET', 404, Date.now() - startTime, ipAddress);
          return errorResponse('NOT_FOUND', 'User not found', 404);
        }

        await logUsage(supabase, keyId, `/${targetUserId}`, 'GET', 200, Date.now() - startTime, ipAddress);
        return successResponse(data);
      }
    }
